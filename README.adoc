...
* Set the `COMPOSE_PROJECT_NAME` variable in `conf/deployer/deploy-k8s.php`
...
You can add stuff here (e.g. GeoLite2 data), all the content of the `/infra` folder
is available to your Docker build process
...
Here we describe how each type of application log  is handled.
...
|Apache logs
|In the container access.log and error.log are symlinked to stdout. This log is available to
Kubernetes log viewer. You can export it to Stackdriver
|One may want to turn off the access.log which is quite verbose, and redundant with Google Analytics.
To that effect add a `RUN rm /var/log/apache2/access.log` instruction in the Dockerfile

|PHP error log
|Stored on an emptyDir volume
|Normally not used often, as most errors will be captured by the Symfony log

|Symfony log
|Monolog logging locally + sending emails via SwiftMailer (Monolog not yet integrated to Mailer)
|Monolog supports many other transports, including Elasticsearch, Redis, syslog, Slack

|msmtp
|Stored on an emptyDir volume
|Normally not used often. Mostly errors connecting to the remote SMTP server, which are not
interesting.
|===

Note: symlinking log files (PHP, msmtp, etc) to `/proc/1/fd/1` (the stdout of the container's PID 1 process,
in this case the `tini` init) usually does not work due to a permission problem.
Other processes can not write into it.

A better, more robust solution would be to syslog these files to a collector in another container.

See the Symfony documentation for a configuration of Monolog to send alerts by email using SwiftMailer.
...
After updating your local `gcloud` libraries, you may encounter permission problems.
Run this:
...
Create a `{PROJECT}` Cloud IAM service account for the application. Add roles:

* Storage > Storage Object Admin
* Cloud SQL > Cloud SQL Client

The service account is named `{PROJECT}@{GCP_PROJECT}.iam.gserviceaccount.com`
...
This service account must be given the following Cloud Storage permissions on buckets:
...
Build your Docker image, then generate and apply deployment manifests.
This is usually done only once, unless you need to change your configuration files or
Docker image.
...
Generate deployment manifests. This is usually done only once.
